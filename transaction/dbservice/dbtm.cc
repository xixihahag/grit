...
void Dbtm::init(EventLoop *loop)
{
    ...
    dbtlClient_->connect();
    dbtlClient_->setConnectionCallback(bind(&Dbtm::onDbtlConnection, this, _1));

    // 连接gtm
    ip = ConfigManager::getInstance()->gtmAddress().c_str();
    port = static_cast<uint16_t>(ConfigManager::getInstance()->gtmPort());
    InetAddress gtmServAddr(ip, port);
    TcpClient *gtmClient_ = new TcpClient(loop, gtmServAddr, "gtm");
    gtmClient_->connect();
    gtmClient_->setConnectionCallback(bind(&Dbtm::onGtmConnection, this, _1));
    ...
    port = ConfigManager::getInstance()->dbtmDbPort();
...
void Dbtm::sendLog(struct transaction *tran)
{
    ...
    auto data_data = builder.CreateVector(data_vec);
    auto dbtl = CreateDbtlMsg(builder, kLog, tran->txid, data_data);
    auto msg = CreateRootMsg(builder, Msg_DbtlMsg, dbtl.Union());
    builder.Finish(msg);
